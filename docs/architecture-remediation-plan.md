...
- A8のencoder checkpointとして、Wavefront OBJ/MTL、binary STLおよびcanonical FDM形式の3MFへfilesystemへ触れない`encode_obj`、`encode_binary_stl`、`encode_3mf`、`encode_mesh_3mf`を追加し、既存`write_*` APIを同じencoderへ委譲した。in-memory bytesとfilesystem wrapperの出力が同一であることをunit testで固定した。DXFおよびglTFのencoder分離は各writer libraryの境界を確認しながら継続する。
