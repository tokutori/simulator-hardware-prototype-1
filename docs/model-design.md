...
retention軸は、別体のrigid bearing blockと外観だけのleaf springを併設せず、inner/outboard両支持板の一体flexureで案内する。各flexureはbearing island、radial方向の変位を許す平行beam、moving/fixed bridgeおよびrigid plateへ荷重を戻すanchor ribから成る。自由状態ではretention軸を理論mesh位置より径方向外側へ0.40 mm置き、組付け時に理論mesh位置まで内向きへ変位させる。FDMへ出す自由状態solidと、assembly表示・干渉検査に使う組付け状態solidは`Body::Compliant`で別々に保持し、後者だけを固定案内梁のcubic変位曲線で近似する。
...
4 unitはroll軸周辺またはコクピ上側のmoving carrierでroll駆動部と一体化する。コクピ直下のcrossbarは撤去し、コクピ下側を床clearance用keep-out volumeとして空ける。moving carrierはroll軸より24 mm上にある左右2本の182 mm長手材と、前後の`RollBearingCarrierEnd`で構成する。このcarrier endは左右railの端部tie、roll bearing boss、ribおよびroll gearbox支持面を一部品へ統合する。接触carriageのtrussはlongitudinal railまで貫通させず、carrier endのoutboard faceで終わる8 × 32 × 8 mm取付padへ接続する。carrier側にも同位置の一体earを設け、1接続につき上下2本のM3x25 bolt、nut、両washerおよび直径3.4 mmの実穴で締結する。上下boltはcarrier中心から±11 mmに置き、12 mm高の長手rail断面を上下から避ける。斜材端はsemantic contact planeでtrimし、外側座面にはwasherとPH2工具の円筒状clearanceを確保する。前後で取付padのlocal Zが異なるためcarriageはfront/rearの別definitionとし、共通plate・bearing・flexure geometryだけを共有する。roll gearboxだけはコクピ前後でroll軸下側に置き、carrier endから面接触するL形bracketで支持する。
...
| 項目 | 初期値 |
| --- | ---: |
| roll shaft | 長さ270 mm / 直径8 mm。bearing部は丸軸、gear/hanger取付部だけD-flat |
| コクピ | 170 × 45 × 45 mm |
...
shaft前後に同じ36T driven gearを固定し、各端を18T output pinionで駆動する。driven gearとhubは一つのFDM definition内でUnionし、gear/hanger取付位置だけshaftに高さ3 mmのD-flat、相手側にnominal 0.15 mm clearanceのD-boreを設ける。旧構成で実keywayなしに別solidへ食い込んでいたkeyとhubは使用しない。bearing位置ではshaftを丸軸のまま保ち、前後を切断しない。shaft–bearing内径とbearing外径–carrier boreは前後各2件のtyped `CylindricalFit`として検証する。bearing envelopeは8 mm shaftに対応する608 seriesの8 × 22 × 7 mmを採用し、寸法はNTN公式product dataを根拠とする（<https://bearingfinder.ntnamericas.com/item/deep-groove-ball-bearings/single-row-radial-ball-bearings/608>）。nominal modelではshaft–bearing内径と前側外輪–carrier boreを0 mm radial clearanceとし、後側外輪だけに下記のslide clearanceを与える。前後retainerは各端3本のM3x20 bolt、nutおよび両washerでcarrierへ締結する。前側外輪の両面contact、後側外輪のinboard contactとoutboard clearance、6件の`FastenedJoint`および別instance間の非交差を型付きrelationとexact Boolean testで検証する。
