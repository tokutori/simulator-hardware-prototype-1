...
8. gear meshはmodule、圧力角、中心距離、軸方向重なり、歯の位相および回転比を一つのrelationとして扱う。
9. animation、collision、floor clearanceおよびmanufacturing outputは、同じassemblyとkinematicsをsource of truthとする。
10. `validate`が成功しない設計から正式な加工データや完成扱いのpreviewを生成しない。
...
各relationは最低限、関係するinstance ID、局所datum、期待する自由度または拘束、寸法公差を持つ。単なるinstance名のpairや「このpairは無視する」という情報にはしない。
...
| Phase | 内容 | 状態 | 主な依存 |
| ---: | --- | --- | --- |
| 0 | 誤った不変条件の撤去とbaseline固定 | 進行中 | なし |
| 1 | typed identityとAssemblyRelation | 未着手 | Phase 0 |
...
- `AssemblyRelation`とrelationごとのdatum/公差型を導入する。
...
- shaft/bore clearanceと軸方向位置決めを`CylindricalFit`で表す。
...
- module、圧力角、center distance、axial overlap、backlash、歯数とmesh種別を検証する。
...
次の作業はPhase 0を完了させた後、Phase 1のtyped identityとAssemblyRelationへ進む。
