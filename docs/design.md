...
ラックの正X側先端には、物体を押すための30 mm（Y）×20 mm（Z）のフラット端面を持つ長さ8 mmの一体パッドを設ける。helical shearによる端部ずれ約2.68 mmより内側までパッドを重ね、全積層でラック本体と接続する。Z寸法は既存のラック歯幅20 mmと同一にして垂直方向へ突出させず、横倒し造形時のbuild plate面内Y方向だけを広げる。全長は約166.8 mmとなる。幅広パッドがケースへ入る位置までラックを引き込まないことを試験時の可動範囲とする。
