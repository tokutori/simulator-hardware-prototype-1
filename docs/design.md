...
D大径とD小径は軸方向の隙間を設けず、大径側を下にして直接積層できる形状とする。ハンドル12T/D大径31Tは5.5/3.5 mmとし、長い小径側が大径側を軸方向に覆ってガタがあっても噛み合いを維持する。二段目はD小径12T/B/C大径28Tをともに5.5 mmとし、全歯幅で噛み合わせて歯元荷重を分散する。

B/C大径の下端は従来位置に固定し、上方向だけへ2 mm厚くして歯幅5.5 mmとする。上面は20 mmピニオンの下面と一致し、15 mmラック下面との軸方向隙間は2.5 mmとなる。これはラック上面とtop plate下面の3.0 mmに近く、ラックの実験用可動空間を残しながらB/C平歯車を強化する。B/Cの下側ヘリカル歯は2 mm下へ延長し、5.5 mm平歯車の上部へ重ねて直接つなぐ。28T平歯車のroot radius 22.95 mmはヘリカルピニオンのtip radius約22.71 mmより大きいため、延長歯の最下層を全周で支持できる。接続用の円錐台テーパーは設けない。
